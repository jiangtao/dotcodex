...
test('example state points at the repo-local prompt file', () => {
  const path = new URL('../examples/state.json', import.meta.url)
  assert.equal(existsSync(path), true)
  const state = JSON.parse(readFileSync(path, 'utf8'))
  assert.match(state.promptFile, /dotcodex\/examples\/prompt\.md$/)
})
